...
export class AppService {
    ...
    async getTicketsList(client: ClientRequest, lang = 'ru', is_bot: boolean = false): Promise<Array<GetTicektsResponse>> {
        ...
        categories = categories.filter(el => {
            if (['99eae575-12ae-48bb-8470-fd9e214b004e', '5d8ae7ad-6061-425e-a9cc-355d2b4687b0'].includes(user?.chat_id) && el.id == 9) {
                return el
            } else if (el.id != 9) {
                return el
            }
    ...
    async upload(file: Express.Multer.File, data: SendFileDto, client, lang): Promise<any> {
        ...
        let createdMessage = await this.prisma.messages.create({
            data: {
                user_id: usersData.id, is_answer: is_answer, operator_id: is_answer == 1 ? client.user_id : null, content_type: contentType, ticket_id: Number(data.ticket_id), message: newMessage
            },
            select: {
                id: true,
                created_at: true,
