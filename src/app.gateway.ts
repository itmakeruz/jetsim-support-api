...
import {
  ...
  editMessageDto,
  sendMessageDto,
} from './message-hendler/dto/sendMessageDto';
import { ZodValidationPipe } from './message-hendler/socket-validator-pipe/validator-service';
import {
  DeleteMessageData,
  EditMessageData,
  SendMessageData,
} from './message-hendler/socket-validator-pipe/schema';
...
export class SocketGateway implements OnGatewayConnection, OnGatewayDisconnect {
  ...
  private async jwtVerify(client) {
    try {
      ...
      this.server
        .to(client.id)
        .emit('exception', { status: 401, message: 'Unauthorized' });
      return false;
    }
  }
}
